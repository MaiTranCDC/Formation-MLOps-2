...
def predict_endpoint():
    ...
    dicValDef = {"Wind_turbine_name": "R80721", "Date_time": "2017-02-08T08:00:00+01:00", "Ba_avg": 44.99, "Ba_min": 44.99,
    ...
    for k in request.args.keys():
        if k in dicValDef:
            dicValDef[k] = request.args.get(k)
    received_data_df = pd.DataFrame(dicValDef, index=[0])
    features = prepare_features(received_data_df, training_mode=False)
    prediction = predict(features=features, model_path=MODEL_PATH)
